...
router.post('/buy', passport.authenticate('jwt', { session: false }), async (req, res) => {
    ...
    try {
        ...
        if(req.user.Point > item.Price && req.user.Point !== 0) {
            let slots = await getFreeSlots(req.user.UserUID);
            if(slots.length > 0) {
                let chResp = await runPreparedQuery(uq.reduceUserPoints, { diff: item.Price, uid: req.user.UserUID });
                if(chResp.rowsAffected[0] > 0) {
                    let bResp = await runPreparedQuery(uq.pushUserRewardItem, { uid: req.user.UserUID, slot: slots[0], id: item.ItemID, count: item.Count, pc: "Item Mall" });
                    if(bResp.rowsAffected[0] > 0) {
                        ...
                        res.status(200).json({ resultCode: 0, message: `Item ${item.ItemName} x${item.Count} was successfully delivered to ${slots[0]} slot of Bank` });
                    } else {
